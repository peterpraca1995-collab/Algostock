...
async function loadStatus() {
  ...
  document.getElementById("schedule-info").textContent =
    `Posledné vyhodnotenie: ${fmtTime(sched.last_run)} · Ďalšie naplánované: ${fmtTime(sched.next_run)}`;
}
...
async function refreshAll() {
  await Promise.all([loadStatus(), loadHistory(), loadEquity()]);
}

document.getElementById("run-now").addEventListener("click", async (e) => {
  e.target.disabled = true;
  e.target.textContent = "Vyhodnocujem…";
  try {
    await fetch("/api/run-now", { method: "POST" });
    await refreshAll();
  } finally {
    e.target.disabled = false;
    e.target.textContent = "Spustiť vyhodnotenie teraz";
  }
});
